<template>
	<div>
		<div class="center_view">
			<div class="list_search_view">
				...
				<div class="btn_view">
                    ...
                    <el-button class="statis_btn" type="warning" @click="echartClick1" v-if="btnAuth('caipinxinxi','库存统计')">
			...
			>
				<div class="item" v-for="(item,index) in list" :key="item.id">
					...
					<div class="btns">
						...
						<el-button class="edit_btn" type="primary" @click="editClick(item.id)" v-if=" btnAuth('caipinxinxi','修改')">
							<i class="iconfont icon-xiugai4"></i>
							修改
						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(item.id)"  v-if="btnAuth('caipinxinxi','删除')">
							<i class="iconfont icon-shanchu1"></i>
							删除
						</el-button>
						<el-button class="operate_btn" v-if="btnAuth('caipinxinxi','查看评论')" type="warning" @click="commentClick(item.id)">
		...
		<el-dialog v-model="echartVisible" title="统计图" width="70%">
            <div  id="alllimittimesEchart1" style="width:100%;height:600px;"></div>
			<template #footer>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var alllimittimesEchart1 = echarts.init(document.getElementById("alllimittimesEchart1"),'theme');
			context.$http({
				url: `${tableName}/value/caipinmingcheng/alllimittimes?order=desc`,
				method: 'get'
			}).then(res=>{
	...
	//初始化
	const init = () => {
		let urlalllimittimes = 'caipinxinxi/remind/alllimittimes/1'
